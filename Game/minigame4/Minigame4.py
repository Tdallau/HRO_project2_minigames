...
from minigame4.HighScore import *
...
class Spielfigur(pygame.sprite.Sprite):
    def __init__(self, initial_position, size):
        ...
        self.image = pygame.image.load('images/spielfigur_' + str(self.go_state) + '.png').convert_alpha()
...
class Cloud(pygame.sprite.Sprite):
    def __init__(self, initial_position, speed):
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load('images/wolke' + str(randint(1, 2)) + '.png').convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.topleft = initial_position
        self.speed = speed
...
startendfont = pygame.font.SysFont("monospace", 40)
game_over_sound = pygame.mixer.Sound("images/game_over.wav")
pling1 = pygame.mixer.Sound("images/pling1.wav")
pling2 = pygame.mixer.Sound("images/pling2.wav")
pling3 = pygame.mixer.Sound("images/pling3.wav")
pling4 = pygame.mixer.Sound("images/pling4.wav")

screen = pygame.display.set_mode((screen_width, screen_height), 0, 32)

spielfigur_jump = pygame.image.load('images/spielfigur_jump.png').convert_alpha()
spielfigur_jump_goodie = pygame.image.load('images/spielfigur_jump_goodie.png').convert_alpha()
spielfigur_1 = pygame.image.load('images/spielfigur_1.png').convert_alpha()
spielfigur_1_goodie = pygame.image.load('images/spielfigur_1_goodie.png').convert_alpha()
spielfigur_2 = pygame.image.load('images/spielfigur_2.png').convert_alpha()
spielfigur_2_goodie = pygame.image.load('images/spielfigur_2_goodie.png').convert_alpha()
spielfigur_3 = pygame.image.load('images/spielfigur_3.png').convert_alpha()
spielfigur_3_goodie = pygame.image.load('images/spielfigur_3_goodie.png').convert_alpha()
spielfigur_4 = pygame.image.load('images/spielfigur_4.png').convert_alpha()
spielfigur_4_goodie = pygame.image.load('images/spielfigur_4_goodie.png').convert_alpha()
    ...
    while True:
        ...
        if game_over_anim > 0:
            ...
            if game_started & (not game_over):
                ...
                if player_slow_time > 0:
                    screen.blit(myfont.render("Langzaam: " + str(player_slow_time), 1, goodie3_color), (screen_width - 320, 25))
                if player_invincible_time > 0:
                    screen.blit(myfont.render("Ontzichtbaar: " + str(player_invincible_time), 1, goodie4_color),
                                (screen_width - 320, 50))
            else:
                if game_over:
                    ...
                    screen.blit(startendfont.render("Klik op enter om het opniew te proberen", 1, background_color),
                                (80, (screen_height / 2) + 100 - 40))
                    screen.blit(startendfont.render("of op q om terug naar het menu te gaan.", 1, background_color),
                                (80, (screen_height / 2) + 180 - 40))
                    if keys[K_RETURN]:
                        obstacles.empty()
                        goodies.empty()
                        score = 0
                    ...
                    if keys[K_q]:
                        ...
                        s = highscore.checkForHighScore(score)
                        return s

                if not game_started:
                    screen.blit(startendfont.render("ENTER om te starten", 1, obstacle_color), (120, (screen_height / 2) - 80))
                    screen.blit(startendfont.render("SPATIE om te springen", 1, obstacle_color),
